const { addFoundItem, updateFoundItem, getFoundItemDetail } = require('../../../api/foundItem.js');
...
Page({
  ...
  async loadDetail(id) {
    try {
      ...
      // 处理时间：ISO 格式转日期字符串
      let foundTime = item.foundTime;
      if (foundTime && foundTime.includes('T')) {
        foundTime = foundTime.split('T')[0];
      }
  ...
  validateForm() {
    const { title, type, description, location, foundTime, contactName, contactPhone } = this.data.form;
    ...
    if (!location) {
      wx.showToast({ title: '请选择拾获地点', icon: 'none' });
      return false;
    }

    if (!foundTime) {
      wx.showToast({ title: '请选择拾获时间', icon: 'none' });
      return false;
    }
  ...
  async onSubmit() {
    ...
    try {
      ...
      // 处理时间：日期字符串转 ISO 格式
      let foundTime = this.data.form.foundTime;
      if (foundTime && !foundTime.includes('T')) {
      ...
      if (this.data.isEdit) {
        ...
        await updateFoundItem(formData);
        wx.showToast({
          title: '更新成功',
          icon: 'success'
        });
