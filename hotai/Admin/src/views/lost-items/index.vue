    ...
    <el-card shadow="hover">
      ...
      >
        ...
        <el-table-column label="悬赏" width="100" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.reward > 0" type="danger" effect="dark">
              ¥{{ row.reward }}
            </el-tag>
...
import { getLostItemList, deleteLostItem, batchDeleteLostItems, batchAuditLostItems } from '@/api/lostItem'
...
const handleBatchDelete = () => {
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条记录吗？`, '批量删除', {
    ...
    try {
      await batchDeleteLostItems(selectedIds.value)
