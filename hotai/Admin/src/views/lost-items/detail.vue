    ...
    <div v-loading="loading" class="detail-container">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="物品类型">
          <el-tag type="info">{{ detail.type }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="悬赏金额">
          <el-tag v-if="detail.reward > 0" type="danger" effect="dark">¥{{ detail.reward }}</el-tag>
          <span v-else style="color: #999;">无悬赏</span>
        </el-descriptions-item>
        ...
        <el-descriptions-item label="丢失位置" :span="2">
        ...
        <el-descriptions-item label="丢失时间" :span="2">
          {{ detail.lostTime }}
        </el-descriptions-item>
...
import { getLostItemDetail, updateLostItemStatus } from '@/api/lostItem'
...
const fetchDetail = async () => {
  loading.value = true
  try {
    const res = await getLostItemDetail(props.itemId)
...
const handleUpdateStatus = async (status) => {
  try {
    await updateLostItemStatus(props.itemId, status)
